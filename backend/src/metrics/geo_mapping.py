"""
Reconciliation between dataset state names and the US-states GeoJSON.
...
In ``us-states.json`` the stable identifier (the two-letter abbreviation)
sits at the top level of each feature, not inside ``properties``, so the id
lookup reads ``feature.get("id") or feature["properties"].get("id")``.
"""
...
class StateGeoMapper:
    ...
    ----------
    geojson_path : Path
        Location of the US-states GeoJSON supplied with the project.
    ...
    def __init__(
        ...
        # ``id`` may be top-level on the feature (us-states.json) or inside
        # ``properties``. Resolve it once per name here.
        self._id_by_name = {
            feature["properties"][GEOJSON_NAME_PROPERTY]: (
                feature.get(GEOJSON_ID_PROPERTY)
                if feature.get(GEOJSON_ID_PROPERTY) is not None
                else feature["properties"].get(GEOJSON_ID_PROPERTY)
            )
            for feature in self.features
        }
    ...
    def geojson_id(self, geojson_name: str) -> str | None:
        ...
        """
        if geojson_name not in self._id_by_name:
            return None

        value = self._id_by_name.get(geojson_name)
