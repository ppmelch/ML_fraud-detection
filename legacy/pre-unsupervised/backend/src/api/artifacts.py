...
def get_fraud_summary() -> dict:
    """
    Load the observed fraud analytics artifact.

    Returns
    -------
    dict
        Portfolio overview plus breakdowns by hour, weekday and category,
        computed from ``Is_Fraud`` directly.
    """
    return _read_json(ARTIFACTS_DIR / "fraud_summary.json")
...
def get_geojson() -> dict:
    """
    Load the India state boundaries GeoJSON.
